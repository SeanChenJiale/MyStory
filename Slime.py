...
class slime(pygame.sprite.Sprite):
    ...
    def player_collision(self):
         collide = pygame.sprite.spritecollide(self,self.game.usergroup, False) ## last argument is for block to disappear.
         for user in collide:
             user.currenthealth -= self.attack_value
             if self.direction == 0:
                 self.rect.y += TILESIZE/2
             if self.direction == 1:
                 self.rect.y -= TILESIZE/2
...
class slime_health_border(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        self.rect.y = self.game.slime.rect.y + 50 + self.heightpadding
